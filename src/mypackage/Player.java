...
public class Player extends GameObject 
{
    ...
    private static final int SPEED_OR_LENGTH_OF_ANGLE = 8;
    private int angle;
	...
	public Player()
    {
		bitmap = EncodedImage.getEncodedImageResource("player.gif").getBitmap();
        xPos = Display.getWidth()/2;
        yPos = Display.getHeight()-65;
        xVelocity = 0;
        yVelocity = 0;     
        type = TYPE_PLAYER;
        lives = 5;
    ...
    public void update() {
  	   ...
  	   //Need to make Velocity int so we can get the object to stop moving (get it to 0)
    ...
    public void forwardBlaster()
    {
        ...
        xVelocity = xVelocity + (int) Math.floor((Math.sin(angle_radians)) * SPEED_OR_LENGTH_OF_ANGLE);
        yVelocity = yVelocity + (int) Math.floor((Math.cos(angle_radians)) * SPEED_OR_LENGTH_OF_ANGLE);
    }
	...
	public void updateShotsFired(int shotsFired) {
	...
	public void updateEnemiesHit(int enemiesHit) {
		this.enemiesHit = this.enemiesHit + enemiesHit;
	}
    ...
    {
        VG11 vg11 = (VG11)vg;
        _image = VGUtils.vgCreateImage(vg11, bitmap, true, VG11.VG_IMAGE_QUALITY_BETTER);
    }
